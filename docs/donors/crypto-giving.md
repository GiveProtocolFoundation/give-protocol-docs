...
| **Optimism** | ETH, WETH, USDC, USDC.e, USDT, DAI, OP |
| **Moonbeam** | GLMR, WGLMR, DOT, USDC, USDT |
...
2. **Step 1 — Select Network**: Choose the blockchain network you want to donate on (Base, Optimism, or Moonbeam).
...
Every blockchain transaction requires a small fee called **gas**, paid in the network's native token (ETH on Base/Optimism, GLMR on Moonbeam). Gas compensates the network's validators for processing your transaction.

**How much does gas cost?**

On Layer 2 networks like Base and Optimism, gas fees are typically less than $0.01 per transaction. On Moonbeam, fees are similarly low. This is much cheaper than Ethereum mainnet (which can cost several dollars per transaction).

**Do I need to hold gas tokens?**

Yes. You need a small amount of the network's native token in your wallet to pay gas:

- **Base or Optimism**: A fraction of ETH (as little as $0.05 worth is usually enough for many transactions).
- **Moonbeam**: A fraction of GLMR.

If you're donating a stablecoin like USDC, you still need a tiny amount of ETH (or GLMR) in your wallet for the gas fee. Your stablecoin donation amount goes entirely to the charity — gas is a separate charge.
...
| Token approval failed | You may not have enough native token (ETH/GLMR) to pay the gas fee for the approval transaction. Add a small amount and try again. |
